...
class MondayCRMAdapter(CRMProvider):
    ...
    async def _gql(self, query: str, variables: dict | None = None) -> dict:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(MONDAY_API_URL,
                                  json=payload,
    ...
    async def _find_item_by_phone(self, phone: str) -> str | None:
        ...
        """
        try:
            data = await self._gql(q, {"board": str(self._board_id),
                                        "phone": phone})
            items = (data.get("data", {})
    ...
    async def upsert_lead(self, lead) -> str:
        ...
        try:
            ...
            if existing_id:
                ...
                """
                await self._gql(q, {"item": existing_id,
                                    "board": str(self._board_id),
                                    "cols": col_vals})
                logger.info("monday_lead_upserted",
                            item_id=existing_id, phone=lead.phone,
                            action="updated")
                return existing_id
            ...
            logger.info("monday_lead_upserted",
                        item_id=item_id, phone=lead.phone, action="created")
            return item_id

        except Exception as e:
            logger.error("monday_api_error", method="upsert_lead",
                         phone=getattr(lead, "phone", "?"), error=str(e))
            return ""

    async def change_stage(self, lead_id: str, new_stage: str,
        ...
        try:
            ...
            await self._gql(q, {"item": lead_id,
                                 "board": str(self._board_id),
                                 "cols": col_vals})
            logger.info("monday_stage_changed",
                        lead_id=lead_id, new_stage=new_stage, label=label)
        except Exception as e:
            logger.error("monday_api_error", method="change_stage",
                         lead_id=lead_id, error=str(e))

    async def add_note(self, lead_id: str, note: str,
                       author: str) -> None:
        import json
        try:
            q = """
            ...
            logger.error("monday_api_error", method="add_note",
                         lead_id=lead_id, error=str(e))

    async def assign_owner(self, lead_id: str, owner_id: str) -> None:
        logger.info("monday_assign_owner_pending",
                    lead_id=lead_id, owner_id=owner_id)

    async def mark_do_not_contact(self, lead_id: str,
                                  reason: str) -> None:
        logger.info("monday_do_not_contact_pending",
                    lead_id=lead_id, reason=reason)

    async def schedule_reactivation(self, lead_id: str,
                                    not_before) -> None:
        logger.info("monday_reactivation_pending",
                    lead_id=lead_id, not_before=str(not_before))
